                ...
                <div class="col-lg-12">
                    ...
                    <div class="toolbar-shop mb-50">
						...
						<?php 
							...
							if(isset($typeid))
							{	
								$fycx=findGoodsByTypeId($typeid);//调用函数根据商品类型查询
							}else
							{
								$fycx=findGoodsByTypeId($id);//调用函数根据商品类型查询
							}
							$page_num=ceil(count($fycx)/$size);
							$sum=count($fycx);
						?>
                        <div class="page-amount">
                            <p>共有<?php echo $sum;?>件商品</p>
                        </div>
                    </div>
                    ...
                    <div class="row shop-wrapper grid_list">
						<?php
							...
							if(isset($typeid))
							{
								$fycx=findGoodsByType($typeid,$start,$size);//调用函数根据商品类型查询
							}else
							{
								$fycx=findGoodsByType($id,$start,$size);//调用函数根据商品类型查询
							}
							if(count($fycx)==0)
							{
                            ...
                            <ul>
								<?php 
									...
									if($page_id>1 && $page_num>1){
										 echo    "<li class='previous'><a href='?page_id=$prepage&typeid=$id'><i class='fa fa-angle-left'></i> 上一页</a></li>";
									}
									                           
									for($x=1; $x<=$page_num; $x++){
										
											echo   "<li><a href='?page_id=$x&typeid=$id'>".$x."</a></li>";
										}
										
									if($page_id>=1 && $page_num>$page_id){
										 echo   "<li class='next'><a href='?page_id=$nextpage&typeid=$id'>下一页 <i class='fa fa-angle-right'></i></a></li>";
									}
								?>
                            </ul>
                        </div>
